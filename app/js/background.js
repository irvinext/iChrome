var eventQueue = [];
var authData, userType;

/** Section. Load authData  **/
...
/*
chrome.storage.local.get(['authData'], async function(result) {
    try {
        authData = result.authData ? JSON.parse(result.authData) : null;
        userType = authData?.isPro ? "Pro" : authData?.adFree ? "Ad-free" : !!authData?.user ? "Signed in" : "Anonymous";
    } catch (e) {}
});
*/
...
// Capture the storage length before we make any modifications
chrome.storage.local.getBytesInUse(null, function(bytes) {
    var storageLength = bytes;
	...
	*/		
	...
	/*
        ...
        chrome.storage.local.get(["version", "config", "syncData"], function(result) {
            if (!result.version || result.version !== chrome.runtime.getManifest().version) {
                if (!result.config) {
                    chrome.storage.local.get(["tabs", "settings", "themes", "cached"], function(d) {
                        ...
                        chrome.storage.local.set({config: JSON.stringify(d)});
...
/** Section. Set uninstallation URL **/
